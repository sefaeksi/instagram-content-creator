---
name: ssefaeksii Content Creator
category: social-media-marketing
version: 4.0
---

# @ssefaeksii — Icerik Stratejisi

## Kim Bu Adam

- **Meslek:** SAP Danismani (3 yildir, aktif calisma hayati)
- **Sehir:** Istanbul
- **Yas profili:** 20'li yaslarin sonunda, hayatini aktif sekilde insa eden biri
- **Spor rutini:** Pazartesi/Sali/Persembe/Cuma gym | Carsamba + Cumartesi kosu kulubu | 1 gun halisaha = 9 antrenman/hafta
- **Ofis gunleri:** Sali ve Carsamba (diger gunler remote)
- **Kosu kulubu:** Carsamba ve Cumartesi, duzenli bulusma yapiliyor
- **Okuma:** Aktif kitap okuyucu — zihinsel donusum, frekans, kisisel gelisim alanlarinda
- **Giyim:** Rahat, sade, fonksiyonel — fazla gostermeden ozguveni yansitan
- **Kahve:** Gunluk ritual, kafeler gunluk hayatin parcasi
- **Iliski:** 2 yillik iliski bu hafta bitti — toparlanma sureci devam ediyor
- **Takipci:** ~7.200 | Hedef: organik buyume, gercek baglanti

---

## Marka Sesi

"Bu adam hayatini yasamasini biliyor — hem zorluklarla yuzlesiyor hem de ilerliyor."

- **Ton:** Samimi, keskin, dogal. Performatif degil — gercek.
- **Format tercihi:** VLOG onceliginde. Kameraya bakmadan, hayati kaydederek anlatan stil.
- **Anlati:** 1. sahis, deneyimden konusan, vaaz vermeden gostere gostere anlatan.
- **Denge:** Ofis insani + sporcu + dusunur. Tek boyutlu degil.
- **Kacınılacak:** Motivasyon guru pozu, sahte pozitiflik, fazla polished/filter'li estetik.

---

## Benchmark Hesaplar — 10 Referans & Ogrenilenler

Bu hesaplari analiz et, kopyalama — onlardan ogrenilen spesifik taktigi uygula.

### 1. @zedan.mutlu (566K) — Keskin Oz Gelistirme
- **Niche:** Erkek oz gelistirme, disiplin, zihin oyunlari
- **Ton:** Direk, sert, yargisiz. Slogan: "Be sharp. Set the standard. Own the outcome."
- **Format:** Reels — bold text hook + kisa sahne + guclu kapanıs cumlesi
- **Ogrenilenler:** Hook'u tek cumleye indir, izleyiciye "bu benim icin" dedirt. Vaaz yok, tokat gibi bir gerceklik var.
- **Uygulanacak:** Her reelin ilk karesinde ekranda 1 cumle — merak yaratır, kaydırmayı durdurur.

### 2. @marvinbrooks (1M) — Dijital Gunluk
- **Niche:** Lifestyle vlog, eski Royal Navy, Londra bazli, fitness + seyahat
- **Ton:** "A digital diary of the endless pursuit" — hayat bir yolculuk anlatisi
- **Format:** Uzun soluklu vlog, gunluk hayat kesimleri, minimal narration
- **Ogrenilenler:** Hayatı bir "pursuit" olarak frame'lemek — hedef degil, yolculuk. Her video bir gunluk olarak hissettiriyor.
- **Uygulanacak:** Istanbul'u sahne olarak kullan. Sehir + spor + is = kimse bu kombinasyonu gostermıyor.

### 3. @nicorauchenwald — Sinematik Estetik
- **Niche:** Lifestyle, Viyana bazli, yuksek estetik, gunluk hayat
- **Ton:** Sessiz, gozlemci, gorsel anlati agirlikli
- **Format:** Sinematik B-roll + minimal metin, atmosfer yaratma
- **Ogrenilenler:** Gorsel tutarlilik = premium marka algisi. Her kare aynı "duyguyu" veriyor.
- **Uygulanacak:** Istanbul'un erken sabah isigindan, kafelerden, kosu parkurundan estetik B-roll stoku biriktirir.

### 4. @ashtonhall (18M+) — Rutin Icerik Ustasi
- **Niche:** Sabah rutini, disiplin, gunluk yapi
- **Ton:** Extreme disiplin, saat 04:00 kalkis, her ani belgeleme
- **Format:** "Morning routine" serisi — ayni format, her gun, tutarli yapi
- **Ogrenilenler:** Seri format = geri donen izleyici. Reel sonu cliffhanger ya da devam eden anlati.
- **Uygulanacak:** Haftalik spor rutinini seri formata don. Izleyici gunleri takip eder, her bolum bagimsiz ama seriye baglanır.

### 5. @bradleysimmonds (604K) — Spor + Profesyonel Hayat
- **Niche:** Londra PT, gym + kariyer dengesi, British GQ trainer
- **Ton:** Enerjik ama erisebilir, vaaz yok — pratik odakli
- **Format:** Antrenman klibi + hayat snippet'i karısımı, story'de check-in
- **Ogrenilenler:** "Trainer'ın gunu" degil, "trainer olan birinin hayati." Fark buyuk. Kimlik onplanda, spor araç.
- **Uygulanacak:** "SAP danismani olan birinin spor hayati" — bu kimse yok. Bu bosluk @ssefaeksii'nin sesidir.

### 6. @hamzahq (Hamza Ahmed, 500K+) — Erkek Gercekligi
- **Niche:** Erkeklerin gercek problemleri — iliski, disiplin, kimlik, spor
- **Ton:** Raw, direkt, akademik alt yapi ama sade dil
- **Format:** Konusarak anlatan Reel + carousel (ders ozeti)
- **Ogrenilenler:** Erkeklerin ses cikarmadigı konuları acıkca konusmak = cok yuksek paylasim orani. Icimdeki soruyu dısarı vur.
- **Uygulanacak:** "2 yillik iliski bitti" gercekligi, zor gunlerde devam etmek, kimsenin gormedigi hayatin arka yuzü — bunlar paylasım getirir.

### 7. @steven.bartlett (2M+) — Savunmasiz + Basarili
- **Niche:** Girisimci hayati, kisisel gelisim, Diary of a CEO
- **Ton:** Savunmasiz storytelling + somut basari = guvenilir kombinasyon
- **Format:** Podcast klibi + kisisel hikaye Reel + carousel
- **Ogrenilenler:** Zaafiyet gostermek gucu azaltmaz. "Bu benim icin zor" demek en yuksek engagement'i getirir.
- **Uygulanacak:** Zorlu sabahlar, motivasyon kaybi, yine de gitmek — bunlari gizleme, goster.

### 8. @lewishowes (3M+) — Spor Altyapili Kisisel Gelisim
- **Niche:** School of Greatness — eski sporcu, kisisel gelisim, is dunyasi
- **Ton:** Ilham verici ama gercekci, deneyim bazli
- **Format:** Kisa motivasyon Reel + carousel (5 ders formatı) + podcast snippet
- **Ogrenilenler:** Spor geri plani + kisisel gelisim = credibility. Aktif hayat yasayan birinin tavsiyeleri daha inandirici.
- **Uygulanacak:** Spor disiplini = is disiplini = hayat disiplini. Bu uclu her zaman icerige donusebilir.

### 9. @alex.sless (432K) — Gunluk Rutin Vlog
- **Niche:** Lifestyle vlog, gunluk rutin, sehirde yasam
- **Ton:** Rahatlatici, izlenesi, normalize edici erkek ozen
- **Format:** DITL (Day in the Life) vlog — kisa kesimler, gunun dortlu-bölümü
- **Ogrenilenler:** Rutin icerik normalize eder ve bagimlılik yaratır. Izleyici "bu benim gunceme ne kadar benze yor" diyerek takip eder.
- **Uygulanacak:** Istanbul'da bir SAP danismaninin DITL — sabah antrenmanı, metro/yol, toplanti, aksam kosu. Bu gunde 4 clip yeter.

### 10. @marcoborges (500K+) — Disiplin Anlatis
- **Niche:** Erkek lifestyle, spor, zihinsel guc, sadelik
- **Ton:** Minimal, guclu, kendinden emin — gosteris yok
- **Format:** Tek konu, derin anlati Reel. Carousel: "X gunden ogrendiklerim"
- **Ogrenilenler:** Az sey, derin anlat. 5 konuyu yuzeysel ele almak yerine 1 konuyu tam bitir.
- **Uygulanacak:** Her reel tek bir gercek ders. "Bugunkü antrenmanı atlayabilirdim ama gitmedim — iste nedeni" = tek konu, tam anlati.

---

## Icerik Sutunlari (6 Farkli Eksen)

### 1. Spor Makinesi
Yogun spor rutiniyle yasayan birinin gercek hayati. Sadece estetik degil — zihinsel savaslar, yorgunluk, yine de gitmek.
- **Format:** Reel (vlog kesiti), Story (gunluk check-in)
- **Alt konular:**
  - Sabah antrenmani vlogu (saat 06:00-07:00 arasi Istanbul'da)
  - Halisaha antrenmani — takim, rekabet, eglence
  - Kosu kulubu bulusmasi — sehirde kosmak, topluluk
  - PR kirmak, hedef koymak, geri dusmek ve kalkmak
  - "Neden bu kadar cok antrenman yapiyorum" — zihinsel aciklama
  - Spor + is hayati dengesi — nasil mumkun oluyor
- **Hooks:** "Bu sabah gitmek istemedim. Gittim. Iste fark.", "Spor yaparken kimsenin konusmadigı bir sey var.", "Halisaha antrenmani her seyden farkli — iste nedeni."

### 2. SAP Danismani Hayati
Beyaz yaka gercekligi — ofis, toplanti, proje, kahve arasi. Izleyicinin cogu bu hayati merak ediyor ya da yasıyor.
- **Format:** DITL Reel (ofis gunu), Carousel (ne ogrendim), Story snippet
- **Alt konular:**
  - Ofis vlogu — bir SAP danismaninin gunu (saat 07:00'den 19:00'a)
  - SAP nedir, bu adam aslinda ne yapiyor (merak edenlere, teknik degil insani)
  - Is-spor-kisisel hayat nasil dengede tutuluyor
  - Kariyer 3. yilinda neredeyim, nereye gidiyorum
  - Toplantidan kosa, kosan sonra tekrar ofise — ayni gun
  - Remote gun vs ofis gunu — fark ne
- **Hooks:** "Ofis hayatinin kimsenin gostermedigi tarafi bu.", "3 yillik kariyer bana su seyi ogrettti.", "Remote gun vs ofis gunu — hangisi daha verimli, deneyimledim."

### 3. Istanbul Vlogu
Istanbul'da aktif bir hayat yasayan birinin gozunden sehir. Spor + is + gunluk yasam.
- **Format:** Reel (vlog kesiti, B-roll agirlikli), Story (spontane an)
- **Alt konular:**
  - Sabah rutini Istanbul'da (spor + kahve + yol + ofis — tek reel, gunun baslangici)
  - Kosu guzergahlari — Bogazici, parklar, sahil, kopru altlari
  - Favori kafe mekanları — calisma ortami veya dinlenme noktasi
  - Haftasonu vs. hafta ici Istanbul — ayni sehir, farkli his
  - Erken sabah Istanbul'u — kimsenin gormedigi sehir
- **Hooks:** "Istanbul'da sabah 6'da kosan biri olmanin farki.", "Sehrin en guzel saati bu.", "Bu kafede her sabah aynı seyi siparis ediyorum — iste neden."

### 4. Zihinsel Donusum & Kisisel Gelisim
Okumak, dusunmek, bakis acisi degistirmek. Sahte motivasyon degil — gercek icsel donusum.
- **Format:** Carousel (ders ozeti, sahsi deneyimle), Reel (kisa insight)
- **Alt konular:**
  - Okudugundan cikardıgı somut dersler — kitap adi degil, ders on planda
  - Spor disiplininin zihinsel etkileri (somut ornekler)
  - 3 yillik is hayatindan hayat dersleri — iyi ve kotu
  - Iliski bittikten sonra kendini yeniden insa etmek
  - Rutinin insan uzerindeki donusturucu gucu — kendi orneginden
  - "X gunden ogrendiklerim" format — spesifik sure ve somut dersler
- **Hooks:** "Son okuduğum kitap su seyi kalici olarak degistirdi.", "Rutini oturtmanin kimsenin anlatmadigi tarafi bu.", "Zihinsel yorgunluk fiziksel yorgunluktan farkli — bunu yasayarak ogrendim."
- **NOT:** Belirli kitap adi gecirme — ders ve kisisel deneyim on planda olsun.

### 5. Kirılgan Ama Guclu Anlar
Gercek anlarin paylasildigi yer. Iliski acisi, yeniden baslamak, zor gunler, yine de devam etmek. Bu sutun en yuksek etkiyi yaratir.
- **Format:** Reel (vlog monolog, direkt kameraya), Story (kisa durustluk)
- **Alt konular:**
  - 2 yillik iliskinin bitmesi — durüst bir paylasim (zamanlama kullaniciya gore)
  - Zor bir gun nasil gecti — ne hissedildi, ne yapildi
  - "Biraktim ama geri dondüm" anlari — spor, is, her sey
  - Kendine karsi durüst olmak
  - Yalniz olmak vs. yalniz hissetmek
  - Motivasyon olmadigi gunlerde ne oluyor — gercekci anlati
- **Hooks:** "Bu hafta hayatimda buyuk bir sey degisti.", "Herkes iyi gunden bahsediyor, ben kotu gunumu anlatarim.", "Kendime karsi durüst olmanın ne kadar zor oldugunu simdi anlıyorum."

### 6. Challenge & Commit (Seri Format)
Kendini bir seye adamak, bunu kamuoyuna acik yapmak ve hesap vermek. Seri format izleyicinin geri donmesini saglar.
- **Format:** Reel serisi (her bölüm bagımsız ama seriye baglanıyor), Story (gunluk update)
- **Alt konular:**
  - 75 HARD (aktif oldugunda — gun bazli gunluk icerik)
  - 30 gun challenge'lari — spor, uyku, dijital detoks, okuma
  - "X gun boyunca Y yaptim, iste sonuc" formatı
  - Haftalik antrenman hesap verme — tamamlanan spor haftasi recap
  - "Simdi neredeyim" serisi — ayda bir ilerleme guncellemesi
- **Hooks:** "Bugun X'in N. gunundeyim.", "Bu ay kendime bir taahhut verdim.", "Son 30 gunde su degisti."

---

## 2025 Algoritma Stratejisi

- **Reels birinci:** Organik erisim icin haftalik 3-4 Reel sart. Reel = kesfet = yeni izleyici.
- **Tamamlanma orani kritik:** Izlenme suresi ne kadar yuksekse, algoritma o kadar yayıyor. Hook kadar son kare de onemli — izleyiciyi bitisine cek.
- **Seri format guc:** Part 1/2/3 veya haftalik devam eden icerik, geri donen izleyici yaratır ve algoritma bu pattern'i oduller.
- **Trending audio:** Her reelde trend ses kullan. Ses trendini gorur gormez 24-48 saat icinde yayınla — erken kullaniciya en yuksek boost verilir.
- **Carousel 2025 stratejisi:** Az metin, cok hikaye. Kisisel fotograf + 1-2 cumle ders. Kaydetme orani yuksek = algoritma severler.
- **Stories 50-30-20 kurali:**
  - %50 Deger (ipucu, ders, gunluk gerceklik)
  - %30 Etkilesim (anket, soru, slider)
  - %20 Baglanti (arka plan, kisisel an, BTS)
- **Gonderi sikligı:** Haftada 3-4 Reel + 2-3 Story serisi + 1 Carousel hedef.

---

## Hook Kurallari

- **Ilk 3 saniye her seyi belirler:** Izleyicilerin %65'i ilk 2-3 saniyede izlemeyi bırakiyor ya da devam ediyor.
- **Gorsel + Metin + Ses uclusu:** Ekranda 1 guclu cumle + sahneyi destekleyen ses + dikkat ceken gorsel acilıs.
- **Sayi kullan:** "3 yil", "2 yillik", "saat 06:00", "30 gun", "haftada X kez" — rakamlar durduruyor.
- **Catısma kur:** Spor + ofis, yorgunluk + devam etmek, iliski acisi + ilerleme, istemiyorum + gittim.
- **Soru degil, ifade:** "Neden bu kadar cok spor yapiyorum?" yerine "Bu hafta her antrenmanı tamamladım — iste fark."
- **Kisisel bas:** "Ben" ile basla, izleyiciye dokunan gerceklik ile bitir.
- **En iyi anlara zıpla:** Reelin en sarsici/etkileyici karesi ile ac, ardindan anlat. Warm-up ile baslamak izleyiciyi kacırır.

### Kanitlanmis Hook Formulleri (2025)
- "[Sayi] [eylem] yaptim — iste [somut sonuc]."
- "Kimsenin anlatmadıgı [konu] gercegi."
- "[Zor an] — [ne yaptigim]."
- "Bu [sure] benim icin her seyi degistirdi."
- "[Beklenti] zannediyordum. [Gerceklik] ogrendim."
- "Bugun [istemiyordum/zorlandım] — ama [yaptım]. Iste neden."

---

## Caption Stratejisi

- **Mikro-blog:** 3-5 cumle, sahsi, deger katan. Vaaz yok — deneyim var.
- **Yapi:** Hook cumlesi → kisisel gerceklik → tek cikartılacak ders → CTA
- **IG SEO kelimeleri:** spordisiplini, sapdanismani, istanbulda yasam, kisiselgelisim, kosu, beyazyaka, vlog, gunlukrutin, donusum, erkekstili, lifestyle
- **CTA rotasyonu:** "Sen ne yapiyorsun?", "Kaydet, unut.", "Takip et, yolculugu izle.", "Bu sana da oldu mu?", "Yoruma yaz."
- **Hashtag:** 10-15, nis + orta + kucuk karisimi. Buyuk hashtag kullanma (1M+ — oraya gomulursun).

---

## Haftalik Icerik Ritmi

| Gun | Gercek Hayat | Format | Sutun |
|---|---|---|---|
| Pazartesi | Gym (remote) | Reel | Spor / Hafta baslangici enerjisi |
| Sali | Gym + Ofis | Carousel | SAP hayati / Ofis vlog / Kariyer insight |
| Carsamba | Kosu kulubu + Ofis | Reel | Kosu kulubu bulusmasi / Istanbul vlog |
| Persembe | Gym (remote) | Story serisi | Kirılgan an / Ham icerik |
| Cuma | Gym (remote) | Reel | Zihinsel donusum / Hafta kapanisi |
| Cumartesi | Kosu kulubu | Reel | Kosu vlog / Halisaha / Spor serisi |
| Pazar | Dinlenme (remote) | Story | Hafta sonu rutini / Gelecek haftaya hazirlik |

---

## Kacınılacaklar

- Sahte pozitiflik — "Her sey mumkun!" klisesi yok
- Sadece estetik icin icerik — anlati olmadan gorsel
- Tekrar eden format (art arda 3 Reel veya art arda 3 motivasyon icerigi olmamali)
- Vaaz vermek — yasamasini anlat, ogretme
- SAP iceriklerini teknik tutmak — insani taraf on planda olmali
- Belirli kitap adi vermek — ders ve kisisel deneyim on planda
- Warm-up sahneyle baslamak — en guclu ana zıpla
- Her reeli motivasyon icerigine donusturmek — cesitlilik sart
